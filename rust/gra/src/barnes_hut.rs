...
pub fn brute_force_repulsion(
    ...
    for i in 0..n {
        ...
        for j in (i + 1)..n {
            let dx = positions[j].0 - px;
            let dy = positions[j].1 - py;
            let d2 = dx * dx + dy * dy;
            if d2 < max_dist2 {
                let c = charge / (1.0 + d2);
                forces[i].0 -= c * dx;
...
impl BarnesHut {
    ...
    pub fn compute_repulsion(
        ...
        self.dual_tree_forces(positions, charge, max_dist, theta * theta)
    }
    ...
    fn dual_tree_forces(
        ...
        while let Some((a, b)) = self.stack.pop() {
            let dx = self.com_x[b] - self.com_x[a];
            let dy = self.com_y[b] - self.com_y[a];
            let l2 = dx * dx + dy * dy;
            ...
            if a != b && combined_w * combined_w < theta2 * l2 {
                ...
                for ki in start_a..end_a {
                    ...
                    for kj in j_start..end_b {
                        ...
                        let pdx = positions[j].0 - px;
                        let pdy = positions[j].1 - py;
                        let pl2 = pdx * pdx + pdy * pdy;
                        if pl2 < max_dist2 {
                            let c = charge / (1.0 + pl2);
                            forces[i].0 -= c * pdx;
                            forces[i].1 -= c * pdy;
                            forces[j].0 += c * pdx;
                            forces[j].1 += c * pdy;
