...
def add_sequence(sequence: jnp.array, coord=(0, 0), interval=2):
    for idx in range(0, sequence.shape[1], interval):
        sequence = sequence.at[:, idx, coord[0], coord[1]].set(1.0)
    return sequence
...
class RhythmDataset:
    ...
    def get_batch(self, batch_size: int = 1, sequence_length: int = 16):
        ...
        sequence = add_sequence(sequence, (self.img_size // 2, self.img_size // 2), interval=2)
        sequence = add_sequence(sequence, (self.img_size // 4, self.img_size // 4), interval=5)
