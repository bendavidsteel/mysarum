// Modal analysis: Initialize Chebyshev T_0 and T_1 from state luminance.
...
fn main(@builtin(global_invocation_id) id: vec3u) {
    ...
    let signal = state_to_scalar(node_state[id.x]);
    ...
    if count > 0u {
        ...
        for (var k = start; k < end; k += 1u) {
            sum += state_to_scalar(node_state[adj_list[k]]);
        }
        avg = sum / f32(count);
    }
