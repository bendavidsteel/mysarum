...
// layout(std140, binding=7) buffer t{
// 	float tree[];
// };
...
vec3 avoidWalls(vec3 pos) {
	float wallDist = 0.001;
	vec3 wallForce = vec3(0);
	wallForce.x += resolution.x * wallDist / pos.x;
	wallForce.x -= resolution.x * wallDist / (resolution.x - pos.x);
	wallForce.y += resolution.y * 10 * wallDist / pos.y;
	wallForce.z += resolution.z * wallDist / pos.z;
	wallForce.z -= resolution.z * wallDist / (resolution.z - pos.z);
	return wallForce;
}
...
void main(){
	...
	vec3 force = vec3(0);
	vec3 gravForce = vec3(0., 0.1, 0.) * ((resolution.y - pos.y) / resolution.y);
	force += gravForce;

	vec3 windForce = vec3(windDirection.x, 0, windDirection.y) * windStrength;
	force += windForce;

	// vec3 aheadPos = pos + aheadDir;
	// float spaceVectorFactor = 0.1;
	// int treeVectorIdx = int(aheadPos.x * spaceVectorFactor) + int(aheadPos.y * spaceVectorFactor * resolution.x * spaceVectorFactor) + int(aheadPos.z * spaceVectorFactor * resolution.x * spaceVectorFactor * resolution.y * spaceVectorFactor);
	// float treePresence = tree[treeVectorIdx];
	// if (treePresence > 0.) {
	// 	force += normalize(-aheadDir) * 10;// / dot(sensorDir, sensorDir);
	// }

	// vec3 wallForce = avoidWalls(pos);
	// force += wallForce;
