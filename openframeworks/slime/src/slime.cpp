...
void Slime::setup(int numAgents)
{
	...
	numSpecies = 1;
	allSpecies = vector<Species>(numSpecies, Species());
	// species 1
	allSpecies[0].colour = {255, 255, 255};
	allSpecies[0].moveSpeed = 1.0;
	allSpecies[0].turnSpeed = 0.0;
	allSpecies[0].sensorAngleDegrees = 45;
...
void Slime::update(){
	for (int i = 0; i < numAgents; i++)
	{
		...
		float sensorAngleRad = species.sensorAngleDegrees * PI / 180.0;
