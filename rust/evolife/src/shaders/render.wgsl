...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Force normalize: use default direction if speed is too low
    // Use safe_speed to avoid division by zero (select evaluates both branches)
    ...
    // Soft edge with glow - use stretched distance for glow, unstretched for core
