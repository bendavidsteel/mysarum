...
def main():
    ...
    if method == 1:
        ...
        num_kernels = 5
        num_growth_funcs = 5
        mu_k = jax.random.uniform(key, (num_species, num_species, num_kernels), minval=0.1, maxval=4.0)
        sigma_k = jax.random.uniform(key, (num_species, num_species, num_kernels), minval=0.5, maxval=5.0)

        mu_g = jax.random.uniform(key, (num_species, num_species, num_growth_funcs), minval=0.1, maxval=4.0)
        sigma_g = jax.random.uniform(key, (num_species, num_species, num_growth_funcs), minval=0.5, maxval=5.0)

        w_k = jax.random.uniform(key, (num_species, num_kernels), minval=-0.1, maxval=0.1)
        c_rep = jax.random.uniform(key, (num_species, num_species), minval=10.0, maxval=50.0)
    
    params = Params(mu_k=mu_k, sigma_k=sigma_k, w_k=w_k, mu_g=mu_g, sigma_g=sigma_g, c_rep=c_rep)
    num_points = 1000
    map_size = 24
    points = jax.random.uniform(key, [num_points, 2]) * map_size
