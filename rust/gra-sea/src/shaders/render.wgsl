...
// Minimum-image wrapping for a displacement component
...
fn vs_line(@builtin(vertex_index) vertex_index: u32) -> LineVertexOutput {
    ...
    // 4 vertices per edge: 2 line segments
    // seg 0 (verts 0,1): from_pos → from_pos + min_image_delta (ghost of to near from)
    // seg 1 (verts 2,3): to_pos → to_pos - min_image_delta (ghost of from near to)
    let conn_idx = vertex_index / 4u;
    let sub_idx = vertex_index % 4u;
    ...
    let from_pos = node_pos[from_idx].xy;
    let to_pos = node_pos[to_idx].xy;

    // Minimum-image displacement from → to
    let dx = wrap_delta(to_pos.x - from_pos.x);
    let dy = wrap_delta(to_pos.y - from_pos.y);

    var world_pos: vec2<f32>;
    switch sub_idx {
        case 0u: { world_pos = from_pos; }                              // seg0 start: from
        case 1u: { world_pos = from_pos + vec2(dx, dy); }              // seg0 end: ghost of to near from
        case 2u: { world_pos = to_pos; }                                // seg1 start: to
        default: { world_pos = to_pos - vec2(dx, dy); }                // seg1 end: ghost of from near to
    }

    out.clip_position = vec4(world_to_clip(world_pos.x, world_pos.y), 0.0, 1.0);
    return out;
}
