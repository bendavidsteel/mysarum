...
def find_cluster_representatives(embeddings_2d: np.ndarray,
    ...
    for cluster in unique_clusters:
        ...
        data_path = str(file_paths[representative_idx][1])
...
def process(output_dir='output'):
    ...
    embeddings_2d = np.load(os.path.join(this_dir_path, 'embeddings_2d.npy'))
    cluster_labels = np.load(os.path.join(this_dir_path, 'cluster_labels.npy'))
    file_paths = np.load(os.path.join(this_dir_path, 'file_paths.npy'))
    ...
    for i, rep in tqdm.tqdm(enumerate(representatives), total=len(representatives), desc='Creating animations'):
