...
@group(0) @binding(1) var<storage, read> node_vel: array<vec4<f32>>;
...
@group(0) @binding(5) var<uniform> params: AudioParams;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i = 0u; i < params.num_nodes; i++) {
        let pos = node_pos[i];
        ...
        let speed = length(node_vel[i].xy);
