...
void Physarum::setup(){
	...
	diffuseRate.set("diffuseRate", 0.1, 0.0, 1.0);
	decayRate.set("decayRate", 0.96, 0., 1.0);
	...
	sensorAngleRad.set("sensorAngleRad", 30. * PI / 180., 0., 90. * PI / 180.);
	sensorOffsetDist.set("sensorOffsetDist", 30., 0., 100.);
	...
	allSpecies[0].colour = glm::vec4(0., 0., 1., 1.);
	...
	density.set("density", 1.0, 0.0, 1.0);
	threshold.set("threshold", 0.0, 0.0, 1.0);
...
void Physarum::draw() {
	...
	float camDist = volWidth * 0.7;
	float camX = centreX + (camDist * std::sin(timeOfDay));
	float camY = centreY + (camDist * (0.5 * std::sin(timeOfDay / 3) + 0.5));
	...
	cam.begin();
	volume.drawVolume(centreX, centreY, centreZ, volSize, 0);
	cam.end();
